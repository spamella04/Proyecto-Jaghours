...
class MatchApplicationHandler extends ApplicationHandler
{
    ...
    {
        $output = "<form method='POST' action='" . route('hourrecords.store') . "'>";
        ...
        foreach ($this->applications as $application) {
            ...
            // Checkbox para seleccionar al estudiante
            $output .= '<td>';
            $output .= "<input type='checkbox' name='students[]' value='{$application->student->id}'>";
            $output .= '</td>';
            $output .= '</tr>';
        }
